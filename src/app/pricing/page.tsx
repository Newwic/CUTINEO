    ...
    <main className="min-h-screen overflow-x-hidden bg-[#071016] text-white">
      <div className="mx-auto max-w-7xl px-5 py-7 sm:px-8 lg:px-12">
            ...
            return (
              <article key={planId} className={`relative flex min-w-0 flex-col rounded-3xl border p-6 ${plan.featured ? 'border-teal-300 bg-teal-300/[0.12] shadow-2xl shadow-teal-500/15 lg:-translate-y-3' : 'border-white/10 bg-white/[0.05]'}`}>
                ...
                <div className="mt-7 min-h-[76px]">
                  <strong className="block text-4xl font-black tracking-tight">{formatPlanPrice(planId)}</strong>
                  <span className="text-xs text-slate-400">{plan.monthlyPriceThb === null ? 'ราคาเริ่มต้น · Custom Package' : '/ เดือน'}</span>
                </div>
                <div className="mt-3 rounded-2xl border border-white/10 bg-black/10 p-3 text-sm text-teal-100">
                  <strong className="block text-xl">{plan.aiMessages.toLocaleString('th-TH')}</strong>
                  <span className="text-xs text-slate-300">AI Messages / เดือน</span>
                </div>
                ...
                <ul className="mt-5 grid gap-3 text-sm leading-6 text-slate-200">
                  {plan.marketingFeatures.map((feature) => <li key={feature} className="flex min-w-0 gap-2"><Check size={16} className="mt-1 shrink-0 text-teal-300" /><span className="min-w-0 break-words">{feature}</span></li>)}
                </ul>
                <Link href={planId === 'enterprise' ? '/#contact' : `/register?plan=${plan.name}`} className={`mt-7 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-black ${plan.featured ? 'bg-teal-300 text-slate-950 hover:bg-teal-200' : 'border border-white/15 text-white hover:bg-white/10'}`}>
        ...
        <section className="mx-auto my-12 flex max-w-4xl flex-col items-start justify-between gap-5 rounded-3xl border border-teal-300/25 bg-teal-300/[0.08] p-6 sm:flex-row sm:items-center sm:p-8">
          ...
          <div className="shrink-0 text-left sm:text-right"><strong className="block text-3xl font-black text-teal-200">฿{AI_BOOST.priceThb}</strong><span className="text-xs text-slate-400">+{AI_BOOST.messages.toLocaleString('th-TH')} messages</span></div>
        </section>

        <p className="mx-auto max-w-2xl pb-12 text-center text-xs leading-6 text-slate-500">AI Message คือข้อความตอบกลับที่สร้างโดย AI ข้อความจากลูกค้าไม่นับเป็น AI Message · โควตาจะ reset ตาม Billing Cycle</p>
      </div>
    </main>
  );
}
